...
class FeatureExtract:
    ...
    def get_features(
            ...
            *,
            ...
            save: bool = False,
            picture_feature: bool = False,
            with_label: bool = False,
    ):
        """
        If there are dataset_kappa files, read them
        """
        ...
        def get_data(path):
            if self.dir_path is not None and os.path.exists(f'{self.dir_path}/{path}.zip'):
                ...
                if picture_feature:
                    structure_features = np.empty(len(data_structure), dtype=object)
                    for i, x in enumerate(
                            tqdm(data_structure, "Converting structures to Matrix", total=len(data_structure),
                                 unit='row')):
                        structure_features[i] = structure_to_feature(x)
                    extra_features = np.hstack([extra_features, structure_features.reshape(-1, 1)])
                if with_label:
                    extra_features = np.hstack([extra_features, np.asarray(data_y)[select_indices].reshape(-1, 1)])
                ...
                if select_col:
                    ...
                    index = len(self.columns) - with_label - picture_feature
                    zero_std_columns = check_zero_std(extra_features[:, :index], self.columns)
                    highly_corr_columns = highly_correlated_columns(extra_features[:, :index], self.columns)
                    index_drop = sorted(
...
def get_atomic_bond_length(structure: Structure) -> list:
    nearest_bond_lengths = []

    # Loop through each atom in the structure
    for i, site in enumerate(structure):
        # Get all neighbors within a reasonable large radius to ensure we capture the nearest neighbor
        neighbors = structure.get_neighbors(site, r=10.0)  # Adjust radius if necessary

        # If there are neighbors, find the shortest distance
        if neighbors:
            nearest_distance = min(neighbor[1] for neighbor in neighbors)  # neighbor[1] is the distance
            nearest_bond_lengths.append(nearest_distance)

    return nearest_bond_lengths
...
def get_structure_features(structure: Structure) -> np.ndarray:
    ...
    bond_lengths = get_atomic_bond_length(structure)
    ...
    structure_features = np.array(
        [
            ...
            b,
            c,
            structure.volume / structure.composition.num_atoms,
            structure.density,
            np.mean(bond_lengths),
            np.std(bond_lengths),
            np.max(bond_lengths) - np.min(bond_lengths),
            np.max(bond_lengths),
            np.min(bond_lengths),
            np.average(stats.mode(bond_lengths)[0]),
        ]
    )
...
def read_structure_file(filename: Union[str, Atoms]) -> Structure:
    """
    Read file
    :param filename: POSCAR.vasp | cif | c2db.json | ase.Atoms
    :return:
    """
    structure = None
...
def structure_to_feature(
    ...
    for idx, (symbol, positions) in enumerate(sorted_symbols_positions):
        ...
        cur = [
            ...
            [(element.max_oxidation_state + 1) * 32, pending[0], pending[1]],
            [(element.min_oxidation_state + 3) * 25, pending[0], pending[1]],
        ]
...
def parallel_descriptor(data_structure):
    with ProcessPoolExecutor() as executor:
        extra_features = list(tqdm(executor.map(single_column_descriptor, data_structure), total=len(data_structure)))
    return np.array(extra_features, dtype=np.float32)


def parallel_structure_to_feature(data_structure):
    with ProcessPoolExecutor() as executor:
        structure_features = list(tqdm(executor.map(structure_to_feature, data_structure), total=len(data_structure)))
    return np.array(structure_features, dtype=object)

if __name__ == '__main__':
    from ase.io import read
    s2 = Structure.from_file("POSCAR.vasp")
    atom = read("POSCAR.vasp")
    z_coords = atom.positions[:, 2]

    z_min = z_coords.min()
    z_max = z_coords.max()
    print(z_max - z_min)
    c = atom.cell.cellpar()[2]
    print(c)
